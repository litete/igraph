...
function showDetail2(a, page_num, id) {
    $.ajax({
        type: "post",
        cache: true,
        async: true,
        ...
        success: function (res) {
            ...
            pre.onclick = function () {
                document.location.href = "/JWLetter/index"
            };
            for (var i = 0; i < title.children.length;) {
                title.removeChild(title.children[0]);
            }
...
function indexOnclick(indexId) {
    /*var  word=document.getElementById("word").value;*/
    var indexId = "";
    document.location.href = "/JWLetter/index?id=" + indexId;
    /*	$.ajax({
     type     : "post",
     cache    : true,
     async    : true,
     dataType : "json",
     data	 : "id="+indexId,
     url      : "http://"+document.location.host+"/JWLetter/index"
     success  : function(res){
